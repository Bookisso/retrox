...
export default function Header(props) {
  ...
  function stopNominating() {
    const newRounds = [...rounds];
    newRounds[roundID].nominationDuration =
      Date.now() / 1000 - rounds[roundID].startBlockTimestamp;
    setRounds(newRounds);
  }

  function stopVoting() {
    const newRounds = [...rounds];
    newRounds[roundID].votingDuration = -666666666;
    setRounds(newRounds);
  }
  ...
  return (
    <header className="flex items-center h-20 px-6 sm:px-10 bg-white">
      ...
      <button className="mr-5" onClick={stopNominating}>
        结束提案期
      </button>
      <button onClick={stopVoting}>结束投票期</button>
      {props.search && (
