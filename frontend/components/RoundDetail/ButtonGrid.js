    ...
    <div className="flex items-center p-8 bg-white rounded-xl shadow-md">
      ...
      <div>
        <span className="block text-2xl font-bold">
          {props.nominationData.length}
        </span>
        <span className="block text-gray-500">提案数量</span>
      </div>
    </div>
  );
}
...
function VotingStateCard(props) {
  ...
  const extraTime = Math.round(
    (props.round.votingDuration -
      Date.now() / 1000 +
      props.round.startBlockTimestamp +
      props.round.nominationDuration) /
      86400
  );
  ...
  return (
    <>
      {props.votingState == 3 ? (
        <div
          className="flex items-center p-8 bg-white rounded-xl shadow-md"
          onClick={props.showChartModal}
        >
          ...
          <div>
            <span className="block text-2xl font-bold">投票结果</span>
            <span className="block text-gray-500">点击查看投票结果</span>
          </div>
        </div>
      ) : props.votingState == 1 || props.votingState == 2 ? (
        <div className="flex items-center p-8 bg-white rounded-xl shadow-md">
          <div className="inline-flex flex-shrink-0 items-center justify-center h-16 w-16 text-white bg-gradient-to-r from-green-500 to-blue-500 rounded-full mr-6">
            <svg
              aria-hidden="true"
          ...
          <div>
            <span className="block text-2xl font-bold">
              {extraTime >= 0 ? extraTime : 0} 天
            </span>
            <span className="block text-gray-500">投票剩余时间</span>
          </div>
        </div>
      ) : (
        <div className="flex items-center p-8 bg-white rounded-xl shadow-md">
          ...
          <div>
            ...
            <span className="block text-gray-500">提案剩余时间</span>
          </div>
        </div>
      )}
    </>
...
function BadgeholderCard(props) {
  return (
    <div
      className="flex items-center p-8 bg-white rounded-xl shadow-md"
      onClick={props.showBadgeholderModal}
    >
      ...
      <div>
        <span className="inline-block text-2xl font-bold">
          {props.voteData && Object.keys(props.badgeholderList()).length}
        </span>
        <span className="block text-gray-500">专家数量</span>
      </div>
    </div>
  );
}
...
function FundingCard(props) {
  return (
    ...
    >
      ...
      <div>
        <span className="block text-2xl font-bold">
          {props.round.funding} ETH
        </span>
        <span className="block text-gray-500">资金池</span>
      </div>
    </div>
  );
}
...
export default function ButtonBar(props) {
  return (
    <section className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
      <NominationCard nominationData={props.nominationData}></NominationCard>
      <VotingStateCard
