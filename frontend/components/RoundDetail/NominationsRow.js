export default function NominationsRow(
  onCardClick,
  selectedNomination,
  nominationData,
  voteData
    ...
    >
      <div className="flex flex-col">
        <span className="font-semibold">
          {nomination.projectName.slice(0, 20).trim() +
            (nomination.projectName.length > 20 ? "..." : "")}
        </span>
        <span className="text-gray-600 group-hover:text-gray-200">
          {nomination.projectDescription.slice(0, 25).trim() +
            (nomination.projectDescription.length > 25 ? "..." : "")}
        </span>
      </div>
